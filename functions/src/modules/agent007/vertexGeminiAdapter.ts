...
import { HugoModelAdapter, ModelOutput, HugoModelRequest, HugoModelResponse, ModelAttempt } from "./hugoCore/modelContract";
  ...
  async generate(input: { message: string; name: string; context: any; history: any[]; promptVersion?: string }): Promise<ModelOutput> {
    ...
    try {
      ...
      const generate = async (requestPrompt: string, maxOutputTokens: number) => {
        ...
        if (!response.ok) { console.warn("[Hugo] Vertex response", response.status); attempts.push({ finishReason: `HTTP_${response.status}`, latencyMs: Date.now() - started,
          promptChars: system.length + requestPrompt.length, outputLimit: maxOutputTokens, inputTokens: null, outputTokens: null, cachedInputTokens: null, reasoningTokens: null });
          return { text: null, finishReason: `HTTP_${response.status}`, usage: null, modelVersion: base.modelVersion }; }
        const payload: any = await response.json();
        const candidate = payload?.candidates?.[0];
        const usage = payload?.usageMetadata || null;
        attempts.push({ finishReason: clean(candidate?.finishReason, 40).toUpperCase() || "UNKNOWN", latencyMs: Date.now() - started,
          promptChars: system.length + requestPrompt.length, outputLimit: maxOutputTokens,
          inputTokens: usage?.promptTokenCount ?? null, outputTokens: usage?.candidatesTokenCount ?? null,
          cachedInputTokens: usage?.cachedContentTokenCount ?? null, reasoningTokens: usage?.thoughtsTokenCount ?? null });
      ...
      return { ...base, text: null, attempts, error: first.finishReason || "NO_FINAL_RESPONSE" };
    } catch (error) { const name = error instanceof Error ? error.name : "UnknownError";
      console.warn("[Hugo] Vertex unavailable", name);
      return { ...base, text: null, attempts, error: /abort|timeout/i.test(name) ? "MODEL_TIMEOUT" : "VERTEX_UNAVAILABLE" }; }
  }
}
