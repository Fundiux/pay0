...
export type LearningExperience = {
  ...
  split: LearningSplit; protectedCaseIds: string[]; createdFrom: { memoryId: string; traceId: string | null };
  sourceVersions: { core: string; context: string; memory: string; prompt: string | null };
  createdAt: string; finalizedAt: string | null; supersedesId: string | null;
};
export type LearningEventType = "EXPERIENCE_CREATED" | "CORRECTION_RECORDED" | "OUTCOME_LINKED" | "EXPERIENCE_VERIFIED" | "EXPERIENCE_REJECTED" | "EXPERIENCE_RETRIEVED" | "LEARNING_EFFECT_MEASURED" | "RULE_CANDIDATE_CREATED" | "SPLIT_ASSIGNED";
...
export type LearningRetrieval = { considered: number; selected: LearningExperience[]; rejected: Array<{ experienceId: string; reason: string }> };
