...
export function validateStructuredCandidate(candidate: StructuredCandidate, scope: { evidenceIds: string[]; entityFacts: Array<{ id: string; status?: string; amount?: string }>;
  experienceIds: string[]; allowedActions: string[]; rootAggregateComplete: boolean; evidenceAliases?: Record<string, string> }): { valid: boolean; reasons: string[] } {
  ...
  for (const claim of candidate.claims || []) {
    const evidenceIds = (claim.evidenceIds || []).map(id => scope.evidenceAliases?.[id] || id);
    if (!evidenceIds.length || evidenceIds.some(id => !evidence.has(id) && !scope.experienceIds.includes(id))) reasons.push("CLAIM_EVIDENCE_UNKNOWN");
