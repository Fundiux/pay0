...
export class HugoConversationCore {
  constructor(private readonly router: HugoToolRouter, private readonly model: HugoModelAdapter) {}
  async respond(input: ConversationInput): Promise<ConversationOutput> {
    ...
    const built = await buildHugoContext({ message: input.message, recentEntities: input.recentEntities, memory: input.memory, router: recordingRouter }).catch(error => {
    ...
    const model = input.commandReply ? { text: null, model: "gemini-2.5-flash", modelVersion: "publisher-model", promptVersion: HUGO_PROMPT_VERSION, tokenUsage: null, error: "COMMAND_HANDLED" }
      : await this.model.generate({ message: input.message, name: input.name, context: built.context, history: input.history });
    ...
    return { text, source: model.text ? "MODEL_RESPONSE" : "DETERMINISTIC_FALLBACK", context: built.context, pieces: built.pieces,
      recentEntities: built.recentEntities, toolsRequested: requested, toolsExecuted: executed, model, promptVersion: HUGO_PROMPT_VERSION };
  }
}
