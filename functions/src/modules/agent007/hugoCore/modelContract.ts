...
export type HugoModelRequest = { schemaVersion: typeof HUGO_MODEL_REQUEST_VERSION; task: { userInput: string; intent: string; name: string; profile?: "OPERATOR" | "PROGRAMMER"; scope?: "GLOBAL" | "PAY0"; risk?: "READ" | "WRITE" | "EXTERNAL_SIDE_EFFECT"; complexity?: "LOW" | "MEDIUM" | "HIGH"; requiredCapabilities?: string[] }; conversation: { history: any[]; activeEntity: { type: string; folio: string } | null };
...
export type ModelOutput = { text: string | null; model: string; modelVersion: string; provider?: string; promptVersion: string;
  tokenUsage: { input?: number; output?: number; cachedInput?: number; reasoning?: number } | null; attempts?: ModelAttempt[]; error?: string; routing?: { primaryProvider: string; fallbackProvider?: string; fallbackReason?: string } };
...
export function canonicalModelRequest(input: { message: string; name: string; context: any; history: any[]; promptVersion: string; capability?: HugoModelRequest["generation"]["capability"]; profile?: "OPERATOR" | "PROGRAMMER"; scope?: "GLOBAL" | "PAY0"; routingSignals?: { intent: string; risk: "READ" | "WRITE" | "EXTERNAL_SIDE_EFFECT"; complexity: "LOW" | "MEDIUM" | "HIGH"; requiredCapabilities: string[] } }): HugoModelRequest {
  const context = input.context || {};
  return { schemaVersion: HUGO_MODEL_REQUEST_VERSION, task: { userInput: input.message, intent: input.routingSignals?.intent || String(context.questionIntent || "GENERAL"), name: input.name, profile: input.profile, scope: input.scope, risk: input.routingSignals?.risk, complexity: input.routingSignals?.complexity, requiredCapabilities: input.routingSignals?.requiredCapabilities },
