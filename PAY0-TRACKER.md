...
| PERF-01 | IMPLEMENTADO / VALIDACIÓN | P1 | Frontend / Pagos | `/pagos` reemplazó la suscripción completa de pagos por `listPagos`, una callable autorizada y acotada por `rootId`, rol y cursor; la tabla carga 100 filas por página y permite continuar. `listPagos` está activa en producción y sus índices compuestos están `READY`; Hosting publicó la interfaz SSR `00387-yiy`. | Ejecutar smoke autenticado con más de 100 pagos. |
