...
| REP-01 | IMPLEMENTADO / VALIDACIÓN | P1 | Reportes | Ganancias por cliente y Tiempos operativos ya consultan por `rootId` + `createdAt`, ordenadas y acotadas desde Firestore; se agregaron sus índices. Incidencias de Pagos conserva datos históricos con fechas heterogéneas (`createdAt`/`fechaPago`/`fecha`) y no se modificó para no excluir operaciones. | Desplegar índices y validar rangos; normalizar fecha canónica de Pagos y migrar su reporte antes de cerrar. |
