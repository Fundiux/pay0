...
| REP-01 | IMPLEMENTADO / VALIDACIÓN | P1 | Reportes | Ganancias por cliente y Tiempos operativos ya consultan por `rootId` + `createdAt`, ordenadas y acotadas desde Firestore; se agregaron sus índices. Pagos nuevos guardan `reportDateAt`; `backfillPagoReportDates` está desplegada, opera por lotes de máximo 250, sólo para superadmin y con simulación por defecto. El panel para supervisar y aplicar cada lote está compilado, pero Hosting no pudo publicarse por resolución DNS intermitente de las APIs `firebasehosting.googleapis.com` y `cloudfunctions.googleapis.com`. | Restablecer conectividad de APIs de Firebase, publicar el panel, ejecutar primero simulación y después backfill aprobado por root; desplegar índices y migrar reporte de Pagos a `reportDateAt` para validar rangos antes de cerrar. |
