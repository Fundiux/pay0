...
| AGT-007 | IMPLEMENTADO / VALIDACION | P1 | Hugo Sanchez / Agente 007 | Una sola entidad: para clientes se presenta como Hugo o Hugito; internamente conserva el identificador Agente 007. Ya observa automaticamente eventos operativos seleccionados desde `activityLog` y conserva contexto de raiz, caso, actor, rol y autorizacion. No responde ni ejecuta acciones en esta etapa. | Functions y frontend build correctos; Functions publicadas para Solicitudes, Pagos, documentos de Solicitud y Facturama. Falta smoke autenticado y ampliar observacion a eventos escritos por transaccion/batch o trigger central. |
...
| FAC-01 | IMPLEMENTADO / VALIDACIÓN | P1 | Facturama | PAY0 ya prepara borradores CFDI manuales y automáticos solo para empresas propias. La automatización liga OC/Solicitud, Materialidad y ciclo ingreso→gasto pendiente, sin timbrar producción ni crear gastos reales sin reglas aprobadas. | Smoke con OC de Trostre, validar datos fiscales del receptor, emisión/consulta/cancelación en sandbox con Facturama, CSD/secretos fuera de Git y aprobación explícita antes de timbrado real. |
...
| FAC-01 | IMPLEMENTADO / VALIDACIÓN | P2 | Facturama | Integración sólo para compañías propias: borradores CFDI manuales y automáticos, liga con Solicitud/OC, expediente de Materialidad y auditoría inicial. | Primera compañía propia opera end-to-end en sandbox: validar CFDI, emitir/consultar/cancelar sin producción real, configurar CSD/secretos fuera de Git y confirmar bloqueo para compañías no propias. |
