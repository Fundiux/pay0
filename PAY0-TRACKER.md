...
| REP-01 | IMPLEMENTADO / VALIDACIÓN | P1 | Reportes | Ganancias por cliente y Tiempos operativos ya consultan por `rootId` + `createdAt`, ordenadas y acotadas desde Firestore; se agregaron sus índices. Pagos nuevos guardan `reportDateAt`; la callable `backfillPagoReportDates` prepara o aplica el historial por lotes de máximo 250, sólo para superadmin y con simulación por defecto. | Desplegar Functions/índices; ejecutar primero simulación y después backfill aprobado por root; cambiar reporte de Pagos a `reportDateAt` y validar rangos antes de cerrar. |
