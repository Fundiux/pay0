...
| REP-01 | IMPLEMENTADO / VALIDACIÓN | P1 | Reportes | Ganancias por cliente y Tiempos operativos ya consultan por `rootId` + `createdAt`, ordenadas y acotadas desde Firestore; se agregaron sus índices. Pagos nuevos ya guardan `reportDateAt` (fecha real de pago o creación); el historial conserva fechas heterogéneas y no se modificó para no excluir operaciones. | Desplegar índices y validar rangos; migrar `reportDateAt` por lotes auditables en Pagos históricos y cambiar su reporte antes de cerrar. |
