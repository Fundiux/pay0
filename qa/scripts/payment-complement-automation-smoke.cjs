...
async function run(){
 ...
 await Promise.all([api.enqueueComplement(root),api.enqueueComplement(root+'-2'),api.enqueueComplement(root)]);
 ...
 await Promise.all([api.executeComplement(job,adapter),api.executeComplement(job,adapter)]);assert.equal(posts,1);
 ...
 const day11=new Date(now.getTime()+11*86400000);
 await Promise.all([api.checkComplementDaily(job,day11,adapter),api.checkComplementDaily(job,day11,adapter)]);assert.equal(checks,1);
 assert((await db.doc(`paymentComplementJobs/${job}`).get()).data().overdueAlertedAt);
 await api.checkComplementDaily(job,new Date(day11.getTime()+86400000),adapter);assert.equal(checks,2);
