...
const identity=require('../../functions/lib/modules/paymentApplications/complementRequestIdentity');
...
async function run(){
 ...
 assert.equal(depositFields.readIqRepDepositFields(withoutIndicator).canRequestRep.type,'absent');
 assert.equal(depositFields.readIqRepDepositFields({...withoutIndicator,can_request_rep:true}).canRequestRep.type,'absent','unobserved alias is not accepted');
 global.fetch=async()=>({ok:true,json:async()=>[{...historicalShape.eligibleIndicatorTrue,id:'220483'}]});
 await assert.rejects(provider.preflightIqComplement({rootId:root,provider:'IQ',depositId:'220483',profileId:'profile',actorUid:root,clientId:root},{accessToken:'emulator-only'}),/IQ_REP_REQUEST_ELIGIBILITY_UNVERIFIED/);
 ...
 const duplicate=job+'-duplicate';await db.doc(`paymentComplementJobs/${duplicate}`).set({...(await db.doc(`paymentComplementJobs/${job}`).get()).data(),status:'QUEUED',applicationId:root,attemptedAt:null,requestedAt:null});
 const failing={...adapter,requestIqComplement:async()=>{posts++;throw Error('TIMEOUT');}};
 await api.executeComplement(duplicate,failing);await api.executeComplement(duplicate,failing);assert.equal(posts,1,'duplicate logical identity cannot POST');assert.equal((await db.doc(`paymentComplementJobs/${duplicate}`).get()).data().status,'BLOCKED');
 await db.doc(`paymentComplementJobs/${duplicate}`).delete();
 await db.doc(`iqCredentialProfiles/profile-2`).set({rootId:root,active:true,hasPassword:true,username:'smoke-2'});
 await db.doc(`pagoApplicationIqPlans/${root}`).update({iqExecutionProfileId:'profile-2'});
 await db.doc(`pagoApplicationIqAttempts/${root}`).update({profileId:'profile-2'});
 await db.doc(`iqUserAccess/${root}`).update({iqCredentialProfileId:'profile-2'});
 await db.doc(`pagoAplicaciones/${root}-3`).set({...app,folio:'AP3'});
 await api.enqueueComplement(root+'-3');assert.equal(await jobFor(root+'-3'),job,'profile change reuses logical request');
 await db.doc(`paymentComplementRequests/${follow.complementRequestId(root,root+'-3')}`).delete();
 await db.doc(`pagoAplicaciones/${root}-3`).delete();
 await db.doc(`iqUserAccess/${root}`).update({iqCredentialProfileId:'profile'});
 await db.doc(`pagoApplicationIqPlans/${root}`).update({iqExecutionProfileId:'profile'});
 await db.doc(`pagoApplicationIqAttempts/${root}`).update({profileId:'profile'});
 const createOther=async(suffix,deposit)=>{
   await db.doc(`pagoApplicationIqPlans/${root}-${suffix}`).set({rootId:root,pagoId:root,iqExecutionProfileId:'profile',plan:{pagoIqFolio:deposit}});
   await db.doc(`pagoApplicationIqAttempts/${root}-${suffix}`).set({rootId:root,profileId:'profile',planId:root+'-'+suffix,createdBy:root});
   await db.doc(`pagoAplicaciones/${root}-${suffix}`).set({...app,folio:'AP-'+suffix,iqPlanId:root+'-'+suffix,iqExecutionAttemptId:root+'-'+suffix});
 };
 await createOther('legacy','220485');
 const legacyId=policy.hash(`${root}:IQ:profile:220485`);
 await db.doc(`paymentComplementJobs/${legacyId}`).set({rootId:root,provider:'IQ',depositId:'220485',profileId:'profile',actorUid:root,clientId:root,applicationId:root+'-legacy',status:'UNKNOWN',attemptedAt:stamp.fromDate(now)});
 await api.enqueueComplement(root+'-legacy');assert.equal(await jobFor(root+'-legacy'),legacyId,'legacy profile-keyed job is reused');
 assert.equal((await db.doc(`paymentComplementJobs/${identity.iqRepRequestId(root,'220485')}`).get()).exists,false,'no canonical duplicate beside legacy job');
 await createOther('timeout','220484');await api.enqueueComplement(root+'-timeout');
 const timeoutJob=await jobFor(root+'-timeout');let timeoutPosts=0;
 const timeoutAdapter={...adapter,requestIqComplement:async()=>{timeoutPosts++;throw Error('TIMEOUT');}};
 await Promise.all([api.executeComplement(timeoutJob,timeoutAdapter),api.executeComplement(timeoutJob,timeoutAdapter)]);
 assert.equal(timeoutPosts,1,'double scheduler cannot duplicate C');
 assert.equal((await db.doc(`paymentComplementJobs/${timeoutJob}`).get()).data().status,'UNKNOWN');
 await api.executeComplement(timeoutJob,timeoutAdapter);assert.equal(timeoutPosts,1,'uncertain result cannot retry');
 await db.doc(`paymentComplementJobs/${timeoutJob}`).update({status:'QUEUED'});
 await api.executeComplement(timeoutJob,timeoutAdapter);assert.equal(timeoutPosts,1,'restart or requeue cannot bypass attemptedAt');
