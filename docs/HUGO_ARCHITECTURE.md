...
## PHASE 6 — CONTROLLED REAL-MODEL LEARNING

The canonical dependency map, experiment results, failure matrix and portability gates are in [HUGO_PHASE6.md](HUGO_PHASE6.md). The Core still owns context, policy, state, memory and learning; Gemini is the current external generative adapter. Phase 6 adds measurable token attempts, an explicit context budget, compact experience projection, conflict withholding, supervised supersession, separate training governance, a pure claim validator and a synthetic compiled-rule experiment. It does not enable structured Gemini output, alter production model configuration, approve operational training data or promote a learned rule. The Firestore emulator proved knowledge survival across separate processes. Open reasoning still requires a model; exact complete facts, ambiguity clarification and partial-total safety now have tested model-free paths.
