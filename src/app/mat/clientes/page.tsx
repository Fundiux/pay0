...
import { useMatClientHome } from "@/components/mat/useMatHome";
import { formatMatTrustWindow } from "@/lib/matSecurityPolicy";
...
function getItemCaption(item: MatHomeItem) {
  return String((item as any)?.caption || (item as any)?.empresaNombre || "Operacion PAY0");
}
...
export default function MatClientesPage() {
  const { home, loading, error } = useMatClientHome();

  const trustWindow = formatMatTrustWindow("CLIENT");
  ...
  const identityLabel = home?.clienteNombre || "Cliente";
  const subtitle = home?.linked ? identityLabel : "Conecta desde Telegram para ver informacion real";
      ...
      children: (
        <div className="space-y-4">
          ...
          <MatBalanceCard
            ...
            status={status}
            statusTone={home?.linked ? "ok" : "info"}
            rows={rows}
          />
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <HomeShortcut label="Disponible" value={getMetricValue(metrics, "Disponible")} targetId="inicio" />
          </div>
        </div>
      ),
    },
    {
      ...
      children: (
        <div className="space-y-4">
          ...
          <OperationList title="Solicitudes recientes" items={solicitudes} empty="Aun no hay solicitudes visibles para este cliente." />
        </div>
      ),
    },
    {
      ...
      children: (
        <div className="space-y-4">
          ...
          <OperationList title="Pagos recientes" items={pagos} empty="Aun no hay pagos visibles para este cliente." />
        </div>
      ),
    },
    {
      ...
      children: (
        <div className="space-y-4">
          ...
          <OperationList title="Dispersiones recientes" items={dispersiones} empty="Aun no hay dispersiones visibles para este cliente." />
        </div>
      ),
    },
    {
  ...
  return (
    <MatShell title="Clientes" subtitle={subtitle} identityLabel={identityLabel} badge="Mini App Telegram" activeNav="inicio" navItems={navItems} botScope="CLIENTS">
