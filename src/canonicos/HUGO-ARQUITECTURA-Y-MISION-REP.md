...
## Seguimiento B diario de expedientes históricos

El scheduler existente `checkPaymentComplementsDaily` corre a las 19:00 de `America/Mexico_City`. Además de los jobs de solicitud, selecciona seguimientos IQ con `nextCheckAt` vencido, estado pendiente de lectura y sin job C. La selección está paginada y acotada a diez expedientes por corrida; ordena por vencimiento. Un claim transaccional guarda el día civil de México antes de contactar IQ, de modo que dos ejecuciones concurrentes no hacen dos lecturas del mismo expediente ese día. Cada lectura verifica de nuevo root, aplicación, fuente financiera, perfil, permisos, Master Switch, configuración y cuota B. Esta ruta solo consulta adjuntos, descarga y valida cuando existen; jamás llama a `requestIqComplement`.

AP1C4U1E6 conserva su job C `UNKNOWN` y seguimiento `REQUEST_STATE_UNKNOWN`: el mismo scheduler revisa únicamente B cuando vence `nextCheckAt`. AP2C4U1E6, AP3C4U1E6 y AP4C4U1E6 no tienen job C; se seleccionan por sus seguimientos `PENDING_B` o evidencia de adjunto pendiente. Una ausencia documentada programa la próxima lectura. Una respuesta ambigua o fallo documental aísla solo ese caso y deja la excepción visible en `/hugo`. El indicador de elegibilidad para la siguiente corrida se coloca antes de las 19:00 del día siguiente; la marca diaria impide repetir lecturas dentro del mismo día. `nextCheckAt` indica elegibilidad, no una ejecución a esa hora exacta.
