  ...
  return <div className="assets-theme md:flex">
    <aside className="sticky top-0 hidden h-dvh w-60 shrink-0 flex-col border-r border-[var(--assets-border)] bg-[var(--assets-sidebar)] md:flex">
      <div className="shrink-0 border-b border-[var(--assets-border)] px-5 py-5">
        <p className="text-[9px] font-semibold uppercase tracking-[0.24em] text-[var(--assets-muted)]">PAY0 Platform</p>
      ...
      <nav className="min-h-0 flex-1 space-y-1 overflow-y-auto p-3">
        {nav.map(({ href, label, icon: Icon, exact }) => <Link key={href} href={href} className={`flex items-center gap-3 rounded-lg px-3 py-2.5 text-[13px] transition ${active(href, exact) ? "bg-[var(--assets-accent-soft)] text-[var(--assets-text)]" : "text-[var(--assets-secondary)] hover:bg-white/[0.04] hover:text-white"}`}><Icon size={16}/>{label}</Link>)}
      </nav>
    ...
    <nav className="fixed inset-x-0 bottom-0 z-50 flex justify-around border-t border-[var(--assets-border)] bg-[var(--assets-sidebar)]/95 px-1 pb-[calc(env(safe-area-inset-bottom)+.35rem)] pt-1.5 backdrop-blur md:hidden">
      {nav.map(({ href, label, icon: Icon, exact }) => <Link key={href} href={href} className={`flex min-w-16 flex-col items-center gap-1 rounded-lg px-2 py-1.5 text-[9px] ${active(href, exact) ? "bg-[var(--assets-accent-soft)] text-[var(--assets-text)]" : "text-[var(--assets-muted)]"}`}><Icon size={17}/>{label}</Link>)}
    </nav>
  </div>;
}
