...
export default function PaymentComplementFollowup() {
  ...
  const [automation, setAutomation] = useState({ iqEnabled: false, facturamaEnabled: false });
  ...
  async function configure(key: "iqEnabled" | "facturamaEnabled") {
    ...
    if (!window.confirm(next[key] ? "¿Activar para aplicaciones PPD nuevas? Facturama emitirá CFDI de pago reales si todos los datos son válidos. No se enviará el histórico." : "¿Pausar nuevos envíos? Los complementos solicitados seguirán revisándose.")) return;
    setBusy(true);
  ...
  return <section className="rounded-2xl border border-white/10 bg-slate-900 p-4 text-slate-200">
    ...
    <p className="my-3 text-xs text-amber-200">Aplicaciones PPD nuevas: IQ solicita por depósito confirmado; Facturama emite con datos fiscales validados. IQ se revisa a las 19:00 (Ciudad de México), con aviso tras 10 días sin recibirlo. No se envía WhatsApp ni se solicita el histórico automáticamente.</p>
    <div className="mb-3 flex gap-3 text-xs">{(["iqEnabled", "facturamaEnabled"] as const).map(key => <button key={key} disabled={busy} onClick={() => void configure(key)} className="rounded border border-sky-400/40 px-3 py-2">{key === "iqEnabled" ? "IQ" : "Facturama"}: {automation[key] ? "activo · pausar" : "pausado · activar"}</button>)}</div>
