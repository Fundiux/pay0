...
import { ArrowDown, Bot, Loader2, Menu, MessageCircle, Plus, Send, X } from "lucide-react";
import { usePathname } from "next/navigation";
import { useUserProfile } from "@/lib/useUserProfile";
import { useModuleAccess } from "@/lib/useModuleAccess";
import { listAgent007Messages, listAgent007Recommendations, markAgent007MessagesRead, sendAgent007Message, type Agent007Message, type Agent007Recommendation } from "@/services/agent007";
...
export default function HugoFloatingBubble() {
  ...
  useEffect(() => { if (menuOpen || chatOpen) void load(); }, [load, menuOpen, chatOpen]);
  ...
  useEffect(() => { if (!chatOpen) return; if (nearBottom) { listRef.current?.scrollTo({ top: listRef.current.scrollHeight }); setNewBelow(false); } else if (messages.length) setNewBelow(true); }, [messages, chatOpen, nearBottom]);
  ...
  const send = async () => {
    ...
    try {
      setNearBottom(true);
      const result = await sendAgent007Message(value, "PAY0");
  ...
  return <>
    <div ref={menuRef} className="fixed bottom-6 right-6 z-[101] flex flex-col items-end gap-2">
      ...
      <button type="button" aria-label={menuOpen ? "Cerrar menú de acciones" : "Abrir menú de acciones"} aria-expanded={menuOpen} onClick={() => setMenuOpen((value) => !value)} className="relative grid h-14 w-14 place-items-center rounded-full bg-blue-600 text-white shadow-2xl transition hover:scale-105 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-300">
        {menuOpen ? <X size={25} /> : <Menu size={25} />}{unread > 0 && <span className="absolute -right-1 -top-1 grid h-6 min-w-6 place-items-center rounded-full bg-rose-500 px-1 text-xs">{Math.min(99, unread)}</span>}
      </button>
    </div>
    {chatOpen && <aside aria-label="Conversación con Hugo" className="fixed bottom-24 right-6 z-[100] flex h-[min(660px,calc(100vh-8rem))] w-[min(440px,calc(100vw-2rem))] flex-col overflow-hidden rounded-2xl border border-violet-500/30 bg-slate-950 shadow-2xl">
      ...
      <div ref={listRef} onScroll={(event) => { const node = event.currentTarget; const close = node.scrollHeight - node.scrollTop - node.clientHeight < 80; setNearBottom(close); if (close) setNewBelow(false); }} className="relative min-h-0 flex-1 space-y-3 overflow-y-auto p-4">
        ...
        {newBelow && <button type="button" onClick={() => { listRef.current?.scrollTo({ top: listRef.current.scrollHeight, behavior: "smooth" }); setNearBottom(true); setNewBelow(false); }} className="sticky bottom-2 mx-auto flex items-center gap-1 rounded-full bg-violet-600 px-3 py-1.5 text-xs text-white shadow-xl"><ArrowDown size={13}/> Ir al mensaje más reciente</button>}
      </div>
      {error && <p className="mx-4 mb-2 rounded-lg border border-rose-500/30 bg-rose-500/10 px-3 py-2 text-xs text-rose-200">{error}</p>}
      <footer className="border-t border-slate-800 p-3"><a href="/hugo#attention" className="mb-2 block text-[11px] text-amber-300">{pending.length ? `${pending.length} asunto(s) requieren atención · Ver en Hugo` : "Sin asuntos que requieran atención"}</a><div className="flex items-end gap-2"><textarea value={text} onChange={(event) => setText(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); void send(); } }} rows={1} placeholder="Escribe a Hugo…" className="max-h-28 min-h-10 min-w-0 flex-1 resize-none rounded-xl border border-slate-700 bg-slate-900 px-3 py-2 text-sm text-white outline-none focus:border-violet-400" /><button type="button" aria-label="Enviar mensaje" disabled={!text.trim() || sending} onClick={() => void send()} className="grid h-10 w-10 place-items-center rounded-xl bg-violet-500 text-white disabled:cursor-not-allowed disabled:opacity-40"><Send size={16} /></button></div><p className="mt-2 text-[10px] text-slate-600">Hugo puede observar y proponer; no ejecuta movimientos financieros.</p></footer>
    </aside>}
  </>;
}
