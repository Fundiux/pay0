...
export default function HugoPhase7Review() {
  ...
  const row = rows[index], key = `${row.evalRunId}:${row.caseId}`, saved = reviews[key];
  ...
  return <section className="mt-6 rounded-2xl border border-violet-500/30 bg-slate-900 p-4 text-sm text-slate-300">
    ...
    {saved && <p className="mt-2 text-xs text-slate-400">Estado: {saved.status} · revision {saved.revision} · {new Date(saved.reviewedAt).toLocaleString("es-MX")}</p>}{message && <p className="mt-2 text-xs text-sky-300">{message}</p>}
  </section>;
}
